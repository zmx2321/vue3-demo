      ...
      <div class="ardl_top_cont">
        <div class="adltop_tip">
          <dl>
            ...
            <dt class="mr20">
              {{ bunkDetail.companyName }}
              <span class="msg_cont">{{ bunkDetail.id == null ? warningMsg : '' }}</span>
            </dt>
    ...
    <div class="ardl_btn_wrap">
      <ul>
        ...
        <li v-if="currentBunkInfo.bunkemptyshopcolour === '1'">
          <el-button type="primary" @click="linktoAttract" disabled>智能招商</el-button>
          <!-- <el-button type="primary" @click="globalInfo.linkToRes('htcx')">智能招商</el-button> -->
        </li>
        <!-- 空余 -->
        <li v-if="currentBunkInfo.bunkemptyshopcolour === '-1'">
          <el-button type="primary" @click="kylinktoAttract" disabled>智能招商</el-button>
        </li>
      </ul>
    </div>
  </section>
  ...
  methods: {
    ...
    linktoAttract() {
      ...
      this.$router.push({
        ...
        query: {
          id: this.bunkDetail.id,
          current: 'linqi',
          regionCode: this.bunkDetail.regionCode,
          regionName: this.bunkDetail.regionName
        }
      })
    },

    kylinktoAttract() {
      console.log('ky', this.bunkDetail)
      // console.log(this.bunkDetail.id)
      /* if(!this.bunkDetail.id) {
        this.$message.warning(this.warningMsg)
        return
      ...
      /* this.$router.push({
        ...
        query: {
          // id: row.id,
          // serviceAreaId: this.bunkDetail.serviceAreaId
          id: this.bunkDetail.berthId,
          // id: 66218,
          serviceAreaId: this.bunkDetail.serviceAreaId,
          hasContract,
          addressStr: JSON.stringify(this.addressStr),
        }
      }) */
      this.$router.push({
        path: '/linkResult',
    ...
    getBunkDetail(berthId, serviceAreaId) {
      return new Promise((resolve, reject) => {
        getBunkDetailByCode(berthId, serviceAreaId)
          .then((res) => {
            resolve(res.data)
          })
          .catch((err) => {
